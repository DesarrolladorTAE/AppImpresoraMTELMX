...
object EscPosBuilder {
    ...
    fun lf(lines: Int = 1): ByteArray =
        "\n".repeat(max(1, lines)).toByteArray(CHARSET)
    ...
    fun bold(on: Boolean): ByteArray =
        byteArrayOf(ESC, 'E'.code.toByte(), if (on) 0x01 else 0x00)
    ...
    // m=0 (pin2) / m=1 (pin5)
    // =========================
    fun drawerPulse(m: Int = 0, t1: Int = 25, t2: Int = 250): ByteArray {
        return byteArrayOf(
            ESC, 'p'.code.toByte(),
            (m and 0xFF).toByte(),
            (t1 and 0xFF).toByte(),
    ...
    fun text(text: String): ByteArray {
        val clean = text
            .replace("\r\n", "\n")
            .replace("\r", "\n")
        return clean.toByteArray(CHARSET)
    }
    ...
    private fun bitmapToRasterEscPos(bitmap: Bitmap): ByteArray {
        ...
        val header = byteArrayOf(
            GS, 'v'.code.toByte(), '0'.code.toByte(), 0x00,
            xL, xH, yL, yH
        )
    ...
    private fun isBlack(colorInt: Int): Boolean {
        ...
        if (alpha < 128) return false // transparente => blanco
    ...
    // =========================
    // Arma ticket completo
    // Orden:
    // 1. Logo
    // 2. Texto antes del QR
    // 3. QR
    // 4. Texto después del QR
    // 5. Corte
    // =========================
    fun build(
        textBeforeQr: String,
        textAfterQr: String? = null,
        cut: Boolean = true,
        ...
        qrSize: Int = 8,
        qrEcc: Int = 49,
        logoMaxWidth: Int = 160
    ): ByteArray {
        val out = ByteArrayOutputStream()
        out.write(init())

        if (shouldOpenDrawer) {
            out.write(drawerPulse(m = drawerPin))
        }

        // Logo
        if (!imageBase64.isNullOrBlank()) {
            out.write(alignCenter())
            out.write(imageFromBase64(imageBase64, maxDotsWidth = logoMaxWidth))
            out.write(lf(1))
            out.write(alignLeft())
        }

        // Bloque principal antes del QR
        if (textBeforeQr.isNotBlank()) {
            out.write(alignLeft())
            out.write(text(textBeforeQr))
            out.write(lf(1))
        }
        ...
        // Bloque después del QR
        if (!textAfterQr.isNullOrBlank()) {
            out.write(text(textAfterQr))
            out.write(lf(1))
        }
